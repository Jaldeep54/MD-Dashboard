...
export function CapacityWaterfall({ capacity }: { capacity: CapacityMetrics }) {
  const { theoreticalMW, availableMW, utilizedMW } = capacity;
  const unavailableMW = theoreticalMW - availableMW;
  const unusedAvailableMW = availableMW - utilizedMW;

  const utilizedPct = (utilizedMW / theoreticalMW) * 100;
  const unusedPct = (unusedAvailableMW / theoreticalMW) * 100;
  const unavailablePct = (unavailableMW / theoreticalMW) * 100;

  const segments = [
    {
      key: "utilized",
      label: "Actual Production (Utilized)",
      pct: utilizedPct,
      mw: utilizedMW,
    ...
    {
      key: "unused",
      label: "Available, Not Utilized",
      pct: unusedPct,
      mw: unusedAvailableMW,
    ...
    <div>
      <div className="mb-2.5 flex items-center justify-between text-[12.5px]">
        <span className="font-semibold text-[var(--color-ink-900)]">
          Theoretical Capacity: {formatMW(theoreticalMW, 2)}
        </span>
        ...
        {segments.map((s) => (
          ...
          >
            ...
            <div className="pointer-events-none absolute bottom-full left-1/2 z-20 mb-2 w-max -translate-x-1/2 whitespace-nowrap rounded-md bg-[var(--color-navy-950)] px-2.5 py-1.5 text-[11px] font-medium text-white opacity-0 shadow-lg transition-opacity group-hover:opacity-100">
              {s.label}: {formatMW(s.mw, 2)} ({s.pct.toFixed(1)}%)
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5">
        {segments.map((s) => (
          <div key={s.key} className="flex items-center gap-1.5 text-[12px] text-[var(--color-ink-500)]">
            ...
            {s.label}
            <span className="font-medium text-[var(--color-ink-900)]">{formatMW(s.mw, 2)}</span>
          </div>
        ))}
      </div>
    </div>
